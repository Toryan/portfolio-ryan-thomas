...
When not working in ggplot2, to create small multiples you can create an individual PNG for each country or region. Once they are created, you can assemble them in rows using magick then assmble those in a stack using magick. 
...
Replace axis abbreviations with words, possibly swap the axes so the terms can be read (or angle the words) 
...
Change "eta" to the greek symbol, or replace it with the terf "efficiency" 
...
Possibly label the top one or two lines with the country label in each panel. 
...
Same data plus additional quantitative variables coud be dataset for a co-plot (3 or 4 quantities) or a parallel-coord plot (any number of quantitative variables) 
...
Possibly, look at distribution of events to determine the cut points between bins or possibly a finer division of time into hours? 
...
After the main graph, you might subset the data to show another graph with significant stories. 

Country could be a panel-level category to look at our trade with specific countries. Then the rows would be ordered by the median to see what industries are most notable. 
